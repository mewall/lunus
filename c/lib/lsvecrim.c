/* LSVECRIM.C - Calculate the scattering vector vs. radius for an
		input image.
...
int lsvecrim(DIFFIMAGE *imdiff_in)
{
...
#ifdef DEBUG
  printf("LSVECRIM: wavelength = %g, distance_mm = %g\n",imdiff_in->wavelength,imdiff_in->distance_mm);
#endif
  ...
  for (pidx = 0; pidx < imdiff_in->num_panels; pidx++) {
	...
	radius = (size_t)(sqrtf(rvec.x*rvec.x + rvec.y*rvec.y)/imdiff->pixel_size_mm+.5);
	  ...
	  rf[radius] += (RFILE_DATA_TYPE)sqrtf(ssq);
	  n[radius]++;
