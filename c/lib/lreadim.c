...
int lreadim(DIFFIMAGE *imdiff)
{
    ...
    if (strstr(buf,"TV6 TIFF")==NULL) {
        ...
        if (strstr(buf,"HEADER_BYTES=")!=NULL &&
            ...
            tag = lgettag(buf, "HEADER_BYTES");
            imdiff->header_length = (size_t)atoi(tag);
            free(tag);
            ...
            tag = lgettag(imdiff->header,"SIZE1");
            imdiff->hpixels = (size_t)atoi(tag);
            free(tag);
            tag = lgettag(imdiff->header,"SIZE2");
            imdiff->vpixels = (size_t)atoi(tag);
            free(tag);
            ...
            if (lgettag(imdiff->header,"IMAGE_PEDESTAL")!=NULL) {
                tag = lgettag(imdiff->header,"IMAGE_PEDESTAL");
                imdiff->value_offset = (IMAGE_DATA_TYPE)atoi(tag);
                free(tag);
            } else {
            ...
            tag = lgettag(imdiff->header,"PIXEL_SIZE");
            imdiff->pixel_size_mm = atof(tag);
            free(tag);
            tag = lgettag(imdiff->header,"BEAM_CENTER_X");
            if (tag!=NULL) {
                imdiff->beam_mm.y = atof(tag);
            } else {
                imdiff->beam_mm.x = imdiff->pixel_size_mm*(float)imdiff->hpixels/2.0;
            }
            free(tag);
            tag = lgettag(imdiff->header,"BEAM_CENTER_Y");
            if (tag!=NULL) {
                imdiff->beam_mm.x = atof(tag);
            } else {
            ...
            tag = lgettag(imdiff->header,"DISTANCE");
            imdiff->distance_mm = atof(tag);
            free(tag);
            tag = lgettag(imdiff->header,"WAVELENGTH");
            imdiff->wavelength = atof(tag);
            free(tag);
            tag = lgettag(imdiff->header,"OSC_START");
            imdiff->osc_start = atof(tag);
            free(tag);
            tag = lgettag(imdiff->header,"OSC_RANGE");
            imdiff->osc_range = atof(tag);
            free(tag);
            tag = lgettag(imdiff->header,"BYTE_ORDER");
            if (!strcmp(tag,"little_endian"))
                imdiff->big_endian=0;
            else if (!strcmp(tag,"big_endian"))
                imdiff->big_endian=1;
            else {
