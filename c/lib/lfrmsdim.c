/* LFRMSDIM.C - Calculate the fractional rmsd between two images in different 
 * resolution shells, as compared to the rms of the first image.
...
int lfrmsdim(DIFFIMAGE *imdiff1, DIFFIMAGE *imdiff2)
{
  RCCOORDS_DATA
    r1,
    r2,
  ...
  im1bins = (double*)calloc(25, sizeof(double)); //25 bins of radial width 50 pix
  ...
  for(r1 = 0; r1 < imdiff1->vpixels; r1 += box_size) {
	...
	if (bindex < 25) {
	  ...
	  nbins[bindex]++;
	  //Also keep track of rms of image 1
	  im1diff = (double)vchunk1/(double)nchunk1 - (double)imdiff1->value_offset;
	  im1bins[bindex] += im1diff*im1diff;
	}
      }
    }
  }
  ...
  for (bindex = 0; bindex < 25; bindex++) {
    ...
    rms = sqrt(im1bins[bindex]/nbins[bindex]);
